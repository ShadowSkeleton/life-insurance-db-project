# Jingrui Feng (jf4446) - database systems project part 3 - csv schema validator
...
SQL_FILES = [
    ROOT / "sql" / "inherited" / "schema_final.sql",
    ROOT / "sql" / "inherited" / "bridge_schema.sql",
    ROOT / "sql" / "physical" / "schema_amendments.sql",
]
TARGETS = [
    "Customer", "Product", "BillingAccount", "Account", "AccountMember", "Relation_3", "Contract", "Claim", "ContractParty",
    "Invoice", "WELLNESS_PROGRAM", "WELLNESS_ENROLLMENT", "WELLNESS_ACTIVITY",
    "RISK_IMPROVEMENT", "RATE_VERSION", "POLICY_RENEWAL",
]
BASE_IDS = {
    "Customer": "CustomerID", "Product": "ProductID", "BillingAccount": "BillingAccountID",
    "Account": "AccountID", "AccountMember": "AccountMemberID",
...
BRIDGE_IDS = {
    ...
    "RATE_VERSION": "RateVersionID", "POLICY_RENEWAL": "RenewalID",
}
...
EXPLICIT_FKS = [
    ...
    ("Contract", "IssuedRateVersionID", "RATE_VERSION", "RateVersionID"),
    ("POLICY_RENEWAL", "ContractID", "Contract", "ContractID"),
    ("POLICY_RENEWAL", "NewRateVersionID", "RATE_VERSION", "RateVersionID"),
]
...
def parse_schema():
    ...
    # Part 3 adds two physical Contract dates and uses the verified, deployed
    # nullable rate-version pin that is documented as a post-deployment ALTER.
    for match in re.finditer(
        r"ALTER\s+TABLE\s+(\w+)\s+ADD\s+(\w+)\s+([A-Z]+)(?:\s*\((\d+)(?:\s*,\s*(\d+))?\))?(.*?);",
        sql_without_comments, re.I | re.S,
    ):
        table, column, typ, length, scale, remainder = match.groups()
        # The inherited DDL also uses ``ALTER TABLE ... ADD CONSTRAINT``.
        # Those are handled above; only actual Part 3 column additions belong
        # in the CSV-shape schema.
        if table in tables and column.upper() != "CONSTRAINT":
    ...
    if "Contract" in tables:
        tables["Contract"]["columns"]["IssuedRateVersionID"] = {
            "type": "INT", "length": None, "scale": None,
            "not_null": False, "identity": False,
        }
    return tables
...
def main():
    ...
    parser.add_argument("--csv-dir", type=Path, default=CSV_DIR, help="directory containing the target CSV files")
    parser.add_argument("--output-report", type=Path, default=ROOT / "outputs" / "volume_csv_validation_report.md", help="Markdown report destination")
    ...
    write_report(tables, data, initial_issues, final_issues, args.output_report, notes)
