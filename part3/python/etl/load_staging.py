...
OUTPUT_DIR = PROJECT_ROOT / "data" / "processed" / "staging_load"
SQL_LOAD_FILE = PROJECT_ROOT / "sql" / "load" / "load_staging_local.sql"
...
def parse_env(path: Path) -> dict[str, str]:
    ...
    if missing:
        raise ValueError(f".env is missing required local SQL settings: {', '.join(missing)}")
    return values
...
def build_brfss(load_date: str) -> tuple[list[list[object | None]], dict[str, Counter[str]]]:
    source = CURATED_DIR / "brfss_2024_life_risk_sample_50000.csv"
...
def load_local(container: str, env: dict[str, str]) -> None:
...
def main() -> None:
    ...
    brfss_rows, brfss_counts = build_brfss(args.load_date)
