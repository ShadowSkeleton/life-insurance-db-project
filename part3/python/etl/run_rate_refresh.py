...
PROFILE_PATH = PROJECT_ROOT / "outputs" / "model" / "predicted_risk_by_profile.csv"
CURATED_DIR = PROJECT_ROOT / "data" / "curated"
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "rate_refresh"
...
def settings(*, azure: bool = False) -> dict[str, str]:
    ...
    if missing:
        target = "Azure SQL" if azure else "local SQL Server"
        raise ValueError(f".env is missing {target} settings: {', '.join(missing)}")
    return values
...
def sqlcmd_query(sql: str, *, azure: bool = False) -> list[list[str]]:
    ...
    command = [
        ...
        SQLCMD, "-C", "-S", "localhost", "-U", env["MSSQL_USER"],
        "-d", env["MSSQL_DATABASE"], "-b", "-W", "-s", "|", "-h", "-1", "-Q",
        "SET NOCOUNT ON; " + sql,
    ]
...
def run_sql_file(path: Path) -> str:
    ...
    command = [
        "docker", "exec", "-e", f"SQLCMDPASSWORD={env['MSSQL_SA_PASSWORD']}", CONTAINER,
        SQLCMD, "-C", "-S", "localhost", "-U", env["MSSQL_USER"],
        "-d", env["MSSQL_DATABASE"], "-b", "-W", "-s", "|", "-h", "-1", "-i", container_path,
    ]
    result = subprocess.run(command, check=True, capture_output=True, text=True)
    ...
    *, azure: bool = False, profile_path: Path = PROFILE_PATH,
    ...
    if azure:
        ...
        with tempfile.TemporaryDirectory(prefix="p3-rate-refresh-") as directory:
            ...
            sql_file.write_text(publish_sql(effective_date, "/tmp/dbsys-p3-rate-refresh/risk_stage.csv", "/tmp/dbsys-p3-rate-refresh/rate_stage.csv", wonder_cohort_source_year, loading_factor, undiagnosed_fraction), encoding="utf-8")
...
def main() -> None:
    ...
    print(json.dumps({key: value for key, value in result.items() if key not in {"risk_rows", "derived"}}, indent=2))
